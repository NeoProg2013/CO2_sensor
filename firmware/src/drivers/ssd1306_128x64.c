...
/// @return    true - success, false - error
//  ***************************************************************************
bool ssd1306_128x64_update(void) {
...
static bool ssd1306_send_command(uint8_t cmd, uint8_t data, bool is_data) {
    ...
    if (is_data == true) {
        ...
        return i2c1_write(DISPLAY_I2C_ADDRESS, 0x00, 1, tx_buffer, 2);
    }
    
    // 0x00 - Control byte = Command
    return i2c1_write(DISPLAY_I2C_ADDRESS, 0x00, 1, &cmd, 1);
}
...
static bool ssd1306_send_bytes(uint8_t* data, uint32_t bytes_count) {
    ...
    return i2c1_write(DISPLAY_I2C_ADDRESS, 0x40, 1, data, bytes_count);
}
