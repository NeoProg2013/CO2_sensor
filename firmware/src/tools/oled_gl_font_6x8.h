...
static void font_6x8_init(font_t* font) {
    
    memset(font, 0x00, sizeof(font_t));
    
    font->width = 6;
    font->height = 8;
    for (char i = ' ' - FONT_SYMBOL_OFFSET; i < '~' - FONT_SYMBOL_OFFSET; ++i) {
        font->symbols[i] = &font_6x8_bitmap[font->width * font->height / 8 * i];
    }
    //font->symbols['1' - FONT_SYMBOL_OFFSET] = &font_6x8_bitmap[font->width * font->height / 8 * 1];
}
